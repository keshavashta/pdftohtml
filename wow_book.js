...
wowBook.prototype = {
	...
	*/
			...
			book._hammer.on("pinch.wowbook transformstart.wowbook transformend.wowbook", function(evt){
				...
				if (!pinchStart) {
					if (book.currentFlip && !book.currentFlip.finished) {
						if (evt.preventDefault) evt.preventDefault(); if (gesture.preventDefault) gesture.preventDefault();
						return;
					}
	...
	enterFullscreen : function(elem){
		elem = $(elem || this.opts.fullscreenElement);
		elem.addClass("wowbook-fullscreen-fix");
		requestFullscreen($(elem || this.opts.fullscreenElement)[0]);
	}, // enterFullscreen

	exitFullscreen : function(){
		$(this.opts.fullscreenElement).removeClass("wowbook-fullscreen-fix");
		exitFullscreen();
	}, // enterFullscreen
	...
	positionBookShadow : function(){
		...
		this.bookShadow.css({
			left  : ((noPageOnLeft ? pw : 0)) + containerLeft/correction,
	...
	_setThumbnailsListPosition : function(position, options){
		...
		var book=this;
		if ( this.pdfDoc ) this.thumbnailsList.queue(function(next){
	...
	setPDF: function( url ){
		...
		PDFJS.getDocument( url ).then(function( pdfDoc ){
			...
			pdfDoc.getPage(1).then(function(page){
				...
				book.setDimensions( viewport.width*2*book.opts.pdfScale, viewport.height*book.opts.pdfScale );
			});
		}, function( error ){
	...
	*/
	pdfRenderPage: function( pageIndex, canvas, callback, scale, type ) {
		...
		this.pdfDoc.getPage( pdfIndex ).then(function(page){
			...
			var renderTask = page.render(renderContext).then(function(){
				callback.call( book, book.pages[pageIndex], canvas);
				book.pdfAnnotations(page, viewport, canvas, $(canvas).parent());
	...
	pdfAnnotations: function (pdfPage, viewport, canvas, annotationsDiv) {
		pdfPage.getAnnotations().then(function( annotationsData ){
			viewport = viewport.clone({ dontFlip: true });
			var transformStr = 'matrix(' + viewport.transform.join(',') + ')';
			var data, element, i, l;
			for (i = 0, l = annotationsData.length; i < l; i++) {
				data = annotationsData[i];
				if (!data || !data.hasHtml || data.subtype!='Link' || !data.url) {
					continue;
				}
				...
				rect = PDFJS.Util.normalizeRect([
					rect[0],	view[3]-rect[1]+view[1],
					rect[2], view[3]-rect[3]+view[1] ]);
	...
	controllify : function(controls){
		...
		$(controls.zoomReset).on("click.wowbook", function(){ book.zoomReset({}); return false });
